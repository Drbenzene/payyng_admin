...
function GameReport({ filters }: GameReportProps) {
  const { data: reportData, isLoading } = useReport(filters);

  const gameStats = [
    {
      name: "Total Games Drawn",
      stat:
        reportData?.reduce(
          (acc: number, curr: any) => acc + (curr?.totalGamesPlayed || 0),
          0
        ) || 0,
    },
    {
      ...
      stat:
        reportData?.reduce(
          (acc: number, curr: any) => acc + (curr?.completedGames || 0),
          0
        ) || 0,
    },
    {
      name: "Cancelled Games",
      stat:
        reportData?.reduce(
          (acc: number, curr: any) => acc + (curr?.failedGames || 0),
          0
        ) || 0,
    },
  ];
  ...
  const series = [
    {
      name: "Total Games Drawn",
      data: reportData?.map((item: any) => item.totalGamesPlayed || 0),
    },
    // {
    //   name: "Completed  Draws",
    //   data: reportData?.map((item: any) => item.completedGames || 0),
    // },
    // {
    //   name: "Cancelled  Draws",
    //   data: reportData?.map((item: any) => item.failedGames || 0),
    // },
  ];
  ...
  return (
    <div>
      ...
      <div className="mt-10">
        <p className="text-lg font-semibold flex justify-start space-x-2  text-primary-100">
