...
function WalletReport({ filters }: WalletReportProps) {
  ...
  const financialStats = [
    {
      ...
      stat:
        reportData?.reduce(
          (acc: number, curr: any) => acc + (curr?.totalWalletBalance || 0),
          0
        ) || 0,
    },
    {
      ...
      stat:
        reportData?.reduce(
          (acc: number, curr: any) => acc + (curr?.totalGameBalance || 0),
          0
        ) || 0,
    },
    {
      ...
      stat:
        reportData?.reduce(
          (acc: number, curr: any) =>
            acc + (curr?.totalWithdrawableBalance || 0),
          0
        ) || 0,
    },
  ];
  ...
  const series = [
    {
      name: "Total Users",
      data: reportData?.map((item: any) => item.totalWalletBalance || 0),
    },
  ];
